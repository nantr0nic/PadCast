...
---
![PadCast v0.2.7](screenshots/padcast-current.gif)
...
- **Input**: a SNES or N64 USB gamepad. Non-SNES/N64 controllers will work but may have limited
button support depending on the adapter. Wireless gamepads work with the Windows build but are
currently untested with Linux builds.
...
> This will enable/disable VSync. While VSync is enabled, the Target FPS setting will be ignored. If you
enable VSync, the program will limit the FPS to match your monitor's refresh rate. If you change your
monitor's refresh rate while the program is running, you will need to toggle VSync off and back on to match
the new refresh rate. Please note that VSync may increase CPU usage depending on your specific hardware
and GPU driver settings; generally the higher refresh rate your monitor is set to, the more CPU usage.
...
to register correct button pushes. Press Spacebar to skip a button (keeps its current value). The button
map is saved to the appropriate layout section in config.ini (e.g., `[SNES_ButtonMap]` or `[N64_ButtonMap]`).
"Reset to Default" resets to the factory defaults for the current layout.
- Layout
> Switch between SNES and N64 controller layouts. Selecting N64 will load the N64 controller image and
...
```
Each section has the same key names as before (`DPAD_UP`, `A_BUTTON`, etc.). The N64 section adds
`C_UP`, `C_DOWN`, `C_LEFT`, `C_RIGHT`, and `Z_BUTTON`. The "Reset to Default" option in the menu
resets to per-layout factory defaults.

#### N64-Specific Config Keys
The following keys control N64 joystick behavior:
```
[Gamepad]
...
then set that number to the appropriate button under ```[SNES_ButtonMap]``` or ```[N64_ButtonMap]```.
...
[SVG source files are available in the GitHub repo](controllerSVG/). This includes the base controller
and pressed button images as layers for SNES and N64. If using Inkscape, export each modified layer as
its own .png file. *The file names must match the defaults.* Meaning, the .png for A must be named A.png
and be placed in the correct layout's `pressed/` folder.

The images must:
- be .png files
- use the same names and locations as the default images
- match the default dimensions (960x540)
...
- Some controllers may be mislabeled by the OS or the underlying gamepad library (GLFW/SDL_GameControllerDB). 
	For example, a USB GameCube controller may be labeled as "XBox Controller".
- **N64 Start button** may not be detected by some USB adapters. If your Start button doesn't register after
	remapping, press Spacebar during the remap sequence to skip it. The Start overlay will be disabled
	when its value is 0 (the default).
- **C-buttons and Start sharing indices**: On some adapters, C-buttons may share button indices with standard
	controller inputs (e.g., C-Up and Start both using index 15). The "Reset to Default" option restores
	the factory defaults which should resolve any conflicts.
...
- SNES and N64 controllers are currently supported, with full joystick and C-button support for N64.
	Future versions aim to add GameCube controller support, and eventually Xbox and PlayStation controllers.
